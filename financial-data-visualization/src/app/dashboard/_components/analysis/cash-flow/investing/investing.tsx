...
interface InvestingData {
    ...
    投资活动产生的现金流量净额_环比: number;
    投资活动产生的现金流量净额_同比: number;
}

// 获取数据的函数
const fetchPeriodData = (stockCode: string): Promise<InvestingData[]> =>
    fetchReportData<InvestingData>("cashflow", "period", stockCode);

const fetchQuarterlyData = (stockCode: string): Promise<InvestingData[]> =>
    fetchReportData<InvestingData>("cashflow", "quarterly", stockCode);

const fetchYearlyData = (stockCode: string): Promise<InvestingData[]> =>
    fetchReportData<InvestingData>("cashflow", "yearly", stockCode);

// 添加指标说明常量
const METRIC_DESCRIPTIONS = {
    amount: {
        title: "投资活动现金流量净额",
        formula: "投资活动产生的现金流量净额",
        description:
            "反映企业投资活动的现金流入和流出的净额。负值表示企业处于扩张期，正值可能表示企业在收回投资或减少投资活动。",
    },
    ratio: {
        title: "投资活动现金流量比率",
        formula: "投资活动产生的现金流量净额 / 经营活动产生的现金流量净额",
        description:
            "反映企业投资活动占用经营现金流的程度。比率越大表示投资活动占用的现金流越多。",
    },
} as const;
...
function InvestingContent() {
    ...
    const getData = (type: "amount" | "ratio") => {
        ...
        return sortedData.map((item) => {
            const value = type === "amount" ? item.投资活动产生的现金流量净额 : item.投资活动产生的现金流量净额;
            const 环比 = dataType === "quarterly" 
                ? item.投资活动产生的现金流量净额_环比
                : null;
            const 同比 = item.投资活动产生的现金流量净额_同比;
    ...
    return (
        <Card className="w-full">
            <CardHeader>
                <CardTitle>投资活动现金流量分析</CardTitle>
                <CardDescription className="flex justify-between items-center">
                    <span>{`${startPeriod}-${endPeriod}期间投资活动现金流量分析`}</span>
...
// 修改主组件
export default function InvestingAnalysis() {
    return (
