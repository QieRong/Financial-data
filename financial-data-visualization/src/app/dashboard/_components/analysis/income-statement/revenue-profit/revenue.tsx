...
// 获取数据的函数
const fetchPeriodData = (stockCode: string): Promise<RevenueData[]> =>
    fetchReportData<RevenueData>("income", "period", stockCode);

const fetchQuarterlyData = (stockCode: string): Promise<RevenueData[]> =>
    fetchReportData<RevenueData>("income", "quarterly", stockCode);

const fetchYearlyData = (stockCode: string): Promise<RevenueData[]> =>
    fetchReportData<RevenueData>("income", "yearly", stockCode);
...
const METRIC_DESCRIPTIONS = {
    revenue: {
        title: "营业总收入",
        formula: "营业总收入",
        description: "反映企业在一定会计期间内经营活动的总收入，包括主营业务收入和其他业务收入。是衡量企业经营规模和市场占有能力的重要指标。",
    },
} as const;
...
function RevenueContent() {
    ...
    const getData = () => {
        ...
        return sortedData.map((item) => {
            const value = item.营业总收入;
            const 环比 = dataType === "quarterly" 
                ? item.营业总收入_环比
                : null;
            const 同比 = item.营业总收入_同比;
    ...
    return (
        <Card className="w-full">
            <CardHeader>
                <CardTitle>营业总收入分析</CardTitle>
                <CardDescription className="flex justify-between items-center">
                    <span>{`${startPeriod}-${endPeriod}期间营业总收入分析`}</span>
            ...
            <CardContent>
                <ChartCard
                    ...
                    rotateLabel
                />
                <MetricDescription type="revenue" />
            </CardContent>
        </Card>
    );
}
